...
public class TestNoticeDAO extends DBConn {
	...
	public ArrayList<TestNoticeVO> getList(int start, int end) {
		Map<String, String> param = new HashMap<String, String>();
		param.put("start", String.valueOf(start));
		param.put("end", String.valueOf(end));

		List<TestNoticeVO> list = sqlSession.selectList(namespace + ".list", param);
		return (ArrayList<TestNoticeVO>) list;

	}

	public TestNoticeVO getContent(String nid) {
	...
	public boolean getInsert(TestNoticeVO vo) {
	...
	public void getUpdateHits(String nid) {
		try {
			String sql = "update TESTNOTICE set ehits=ehits+1 " + " where nid=?";
	...
	public boolean getDelete(String nid) {
		boolean result = false;
		int val = sqlSession.delete(namespace+".delete",nid);
	 ...
	 * Update : ���� ����
	 */
	public boolean getUpdate(TestNoticeVO vo) {
		boolean result = false;
